<!--<h3>Currently working on it! Some junk text will be displayed! Kindly ignore it!</h3>-->
<?php
session_start();
                ...
                while ($result_row = mysqli_fetch_object($result_unique_team)) {
                    ...
                    while ($result_row_user = mysqli_fetch_object($result_user)) 
                    {
                        ...
                        if(!empty ($task))
                        {
                            if(!empty ($s_task))
                            {
                                $task_text_temp=implode(",", $task);
                                $s_task_text_temp=implode(",", $s_task);
                                $query_user_2 = "SELECT time,count,wu FROM user_tasks WHERE team_id='$unique_team' AND user_id='$unique_team_user' AND tasks_id IN ($task_text_temp) AND sub_task_id IN ($s_task_text_temp) AND " . $where_2;
                            }
                            else
                            {
                                $task_text_temp=implode(",", $task);
                                $query_user_2 = "SELECT time,count,wu FROM user_tasks WHERE team_id='$unique_team' AND user_id='$unique_team_user' AND tasks_id IN ($task_text_temp) AND " . $where_2;
                            }
                        }
                        else {
                        ...
                        if(!empty ($task))
                        {
                            if(!empty ($s_task))
                            {
                                $task_text_temp=implode(",", $task);
                                $s_task_text_temp=implode(",", $s_task);
                                $query_user_2 = "SELECT time,count,wu FROM user_tasks_ot WHERE team_id='$unique_team' AND user_id='$unique_team_user' AND tasks_id IN ($task_text_temp) AND sub_task_id IN ($s_task_text_temp) AND " . $where_2 ." AND ot_status=1 ";
                            }
                            else
                            {
                                $task_text_temp=implode(",", $task);
                                $query_user_2 = "SELECT time,count,wu FROM user_tasks_ot WHERE team_id='$unique_team' AND user_id='$unique_team_user' AND tasks_id IN ($task_text_temp) AND " . $where_2 ." AND ot_status=1 ";
                            }
                        }
                        else {
                            $query_user_2 = "SELECT time,count,wu FROM user_tasks_ot WHERE team_id='$unique_team' AND user_id='$unique_team_user' AND " . $where_2 ." AND ot_status=1 ";
                        }
                        
                        $result_user_2 = $conn->runsql($query_user_2, $dbcon);
                        $secs_user_ot=0;
                        $tot_work_units_user_ot=0.0;
                        while ($result_row_user_2 = mysqli_fetch_object($result_user_2)) {
                            $secs_user_ot+= strtotime($result_row_user_2->time)-strtotime("00:00:00");
                            $tot_work_units_user_ot+=$result_row_user_2->wu;
                        }
                        //echo round($tot_work_units_user/$secs_user*28800,2);
                        $min_max[$unique_team_user]=@round((($tot_work_units_user+$tot_work_units_user_ot)/($secs_user+$secs_user_ot))*28800,2);
                        //echo "((".$tot_work_units_user."+".$tot_work_units_user_ot.")/(".$secs_user."+".$secs_user_ot."))*28800"."<br>";
                    }
                    ...
                    if(!empty ($task))
                    {
                        if(!empty ($s_task))
                        {
                            $task_text_temp=implode(",", $task);
                            $s_task_text_temp=implode(",", $s_task);
                            $query = "SELECT time,count,wu FROM user_tasks WHERE team_id='$unique_team' AND tasks_id IN ($task_text_temp) AND sub_task_id IN ($s_task_text_temp) AND " . $where_2;
                        }
                        else
                        {
                            $task_text_temp=implode(",", $task);
                            $query = "SELECT time,count,wu FROM user_tasks WHERE team_id='$unique_team' AND  tasks_id IN ($task_text_temp) AND " . $where_2;
                        }
                    }
                    else {
                    ...
                    if(!empty ($task))
                    {
                        if(!empty ($s_task))
                        {
                            $task_text_temp=implode(",", $task);
                            $s_task_text_temp=implode(",", $s_task);
                            $query_ot = "SELECT time,count,wu FROM user_tasks_ot WHERE team_id='$unique_team' AND tasks_id IN ($task_text_temp) AND sub_task_id IN ($s_task_text_temp) AND " . $where_2." AND ot_status=1 ";
                        }
                        else
                        {
                            $task_text_temp=implode(",", $task);
                            $query_ot = "SELECT time,count,wu FROM user_tasks_ot WHERE team_id='$unique_team' AND tasks_id IN ($task_text_temp) AND " . $where_2." AND ot_status=1 ";
                        }
                    }
                    else {
...
<!--                    <td><?php //echo "<b>".@round( (($tot_work_units+$tot_work_units_ot)/($secs+$secs_ot)*28800) ,2 )."</b>&nbsp;(".@round($tot_work_units/$secs*28800,2)."&nbsp;/&nbsp;".@round($tot_work_units_ot/$secs_ot*28800,2).")";?></td>-->
                    <td>
                        <?php 
                            $tot_test_1=$tot_work_units+$tot_work_units_ot;
                            $tot_test_2=$secs+$secs_ot;
                            echo round($tot_test_1/$tot_test_2*28800,2);
                            echo "(".round($tot_work_units/$secs*28800,2)."&nbsp;/&nbsp;";
                            echo round($tot_work_units_ot/$secs_ot*28800,2).")";
                          //echo "(".round( ($tot_work_units/$secs*28800) ,2 )."/".round( ($tot_work_units_ot/$secs_ot*28800) ,2 ).")";  
                        //echo round( (($tot_work_units+$tot_work_units_ot)/($secs+$secs_ot)*28800) ,2 );
                        ?>
                    </td>
...
<?php
            ...
            elseif((!empty ($team)) && (!empty ($task)))
            {
                if(!empty ($s_task))
                {
                    ...
                    $s_task_text_temp=implode(",", $s_task);
                    $query_unique_team_query = "SELECT DISTINCT tasks_id FROM user_tasks WHERE tasks_id IN ($task_text) AND sub_task_id IN ($s_task_text_temp) AND " . $where_2 . " ORDER BY tasks_id ASC";
                }
                else
                {
                    $task_text=implode(",", $task);
            ...
            //echo $query_unique_team_query;
                ...
                while ($result_row = mysqli_fetch_object($result_unique_team)) {
                    ...
                    {
                        $s_task_text_temp=implode(",", $s_task);
                        $query_user="SELECT DISTINCT user_id FROM user_tasks WHERE tasks_id='$unique_task' AND sub_task_id IN ($s_task_text_temp) AND " . $where_2;
                    }
                    else
                    {
                    ...
                    while ($result_row_user = mysqli_fetch_object($result_user)) 
                    {
                        ...
                        {
                            $s_task_text_temp=implode(",", $s_task);
                            $query_user_2 = "SELECT time,count,wu FROM user_tasks WHERE tasks_id='$unique_task' AND sub_task_id IN ($s_task_text_temp) AND user_id='$unique_team_user' AND " . $where_2;
                        }
                        else
                        {
                        ...
                        {
                            $s_task_text_temp=implode(",", $s_task);
                            $query_user_2 = "SELECT time,count,wu FROM user_tasks_ot WHERE tasks_id='$unique_task' AND sub_task_id IN ($s_task_text_temp) AND user_id='$unique_team_user' AND " . $where_2." AND ot_status=1 ";
                        }
                        else
                        {
                            $query_user_2 = "SELECT time,count,wu FROM user_tasks_ot WHERE tasks_id='$unique_task' AND user_id='$unique_team_user' AND " . $where_2." AND ot_status=1 ";
                        }
                        $result_user_2 = $conn->runsql($query_user_2, $dbcon);
                        while ($result_row_user_2 = mysqli_fetch_object($result_user_2)) {
                            $secs_user+= strtotime($result_row_user_2->time)-strtotime("00:00:00");
...
//                    print_r($min_max);
                    ...
                    {
                        $s_task_text_temp=implode(",", $s_task);
                        $query = "SELECT team_id,time,count,wu FROM user_tasks WHERE tasks_id='$unique_task' AND sub_task_id IN ($s_task_text_temp) AND " . $where_2;
                    }
                    else
                    {
                    ...
                    {
                        $s_task_text_temp=implode(",", $s_task);
                        $query = "SELECT team_id,time,count,wu FROM user_tasks_ot WHERE tasks_id='$unique_task' AND sub_task_id IN ($s_task_text_temp) AND " . $where_2." AND ot_status=1 ";
                    }
                    else
                    {
                        $query = "SELECT team_id,time,count,wu FROM user_tasks_ot WHERE tasks_id='$unique_task' AND " . $where_2." AND ot_status=1 ";
                    }
                    $result = $conn->runsql($query, $dbcon);
                    while ($result_row = mysqli_fetch_object($result)) {
                        $tot_count_ot+=$result_row->count;
                        $secs_ot+= strtotime($result_row->time)-strtotime("00:00:00");
                        $team_id=$result_row->team_id;
                        $tot_work_units_ot+=$result_row->wu;
                    }
                    ...
                    <td><?php 
                        $oa_time+=$secs+$secs_ot;
                        echo $tot_time=sectohr($secs+$secs_ot)."(".sectohr($secs)."/".sectohr($secs_ot).")"; ?>
                    </td>
                    <td><?php 
                        $oa_count+=$tot_count+$tot_count_ot;
                        echo $tot_count+$tot_count_ot ?>
                    </td>
                    <td><?php 
                        $oa_wu+=$tot_work_units+$tot_work_units_ot;
                        echo $tot_work_units+$tot_work_units_ot."(".$tot_work_units."/".$tot_work_units_ot.")";?>
                    </td>
                    
                    <td><?php echo round(($tot_work_units+$tot_work_units_ot)/$secs*28800,2);?></td>
                    <td><?php echo $min=min($min_max);
                    echo "&nbsp;(". $user_array[array_search($min,$min_max)].")"?></td>
                    <td><?php echo $max=max($min_max);
                    echo "&nbsp;(". $user_array[array_search($max,$min_max)].")"?></td>
            ...
            elseif(!empty ($team) && !empty ($task))
            {
                ...
                //echo $query;
                    ...
                    while ($result_row1 = mysqli_fetch_object($result_unique_team)) {
                        ...
                        <td><?php echo $tot_count+$tot_count_ot."&nbsp;(".$tot_count."&nbsp;/&nbsp;".$tot_count_ot.")" ?></td>
                        <td><?php echo $tot_work_units+$tot_work_units_ot."&nbsp;(".$tot_work_units."&nbsp;/&nbsp;".$tot_work_units_ot.")"?></td>
                        <td><?php echo round(($tot_work_units+$tot_work_units_ot)/($secs+$secs_ot)*28800,2)."&nbsp;(".round($tot_work_units/($secs+$secs_ot)*28800,2)."&nbsp;/&nbsp;".round($tot_work_units_ot/($secs+$secs_ot)*28800,2).")";?></td>
            ...
            else 
            {
                ...
                else
                {
                	$user_text=implode(",", $user);
                        ...
                        $query_ot = "SELECT team_id,tasks_id,sub_task_id,task_desc,time,count,cf,wu FROM user_tasks_ot WHERE user_id IN ($user_text) AND " . $where_1." AND ot_status='1'";
                        $result_ot = $conn->runsql($query_ot, $dbcon);
                        while ($result_row_ot = mysqli_fetch_object($result_ot)) {
                            $tot_count_ot+=$result_row_ot->count;
                            $secs_ot+= strtotime($result_row_ot->time)-strtotime("00:00:00");
                        ...
                        <td><?php echo $i++; ?></td>
                        <td><?php
                        //echo $user_array[$user] ;
                            for($kk=0;$kk<sizeof($user);$kk++)
                            echo $user_array[$user[$kk]].",&nbsp;";
                                ?></td>
