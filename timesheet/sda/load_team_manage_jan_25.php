...
for($i=0;$i<$size;$i++)
{
    $query_1="SELECT team_id,team_name FROM amz_teams where team_id='$avail_teams[$i]' AND team_deletion='0'";
    ...
    if(mysqli_num_rows($result_1))
    {
        while($result_row_1=  mysqli_fetch_object($result_1))
        {
            ...
            &nbsp;&nbsp;&nbsp;
            <a href="#" data-reveal-id="edit_team_<?php echo $sec_team_id ?>" data-reveal-ajax="./edit_data.php?subject=team&&val=<?php echo $sec_team_id ?>">Edit</a>&nbsp;&nbsp;&nbsp;
            <a href="#" onclick="del_ua('team','<?php echo $sec_team_id?>');">Delete</a>
            </li>
           <div id="edit_team_<?php echo $sec_team_id ?>" class="reveal-modal small" data-reveal>
           <a class="close-reveal-modal">&#215;</a>
        </div>
            <?php $query_2="SELECT task_id,task_name,op_type FROM amz_tasks where team_id='$sec_team_id' AND deletion=0";
            ...
            if(mysqli_num_rows($result_2))
            {
                ...
                while($result_row_2=  mysqli_fetch_object($result_2))
                {
                    ...
                    ?><li id='hide_show_st' data-tooltip aria-haspopup="true" class="tip-right" title="Click the task name to view the sub tasks!" onclick='show_hide(<?php echo $sec_task_id?>);'>[<?php echo $op_type; ?>]&nbsp;&nbsp;<?php echo $sec_task_name=$result_row_2->task_name; ?>
                        &nbsp;&nbsp;&nbsp;
            <a href="#" data-reveal-id="edit_task_<?php echo $sec_task_id ?>" data-reveal-ajax="./edit_data.php?subject=task&&val=<?php echo $sec_task_id ?>">Edit</a>&nbsp;&nbsp;&nbsp;
            <a href="#" onclick="del_ua('task','<?php echo $sec_task_id?>');">Delete</a> 
            </li>
           <div id="edit_task_<?php echo $sec_task_id ?>" class="reveal-modal small" data-reveal>
           <a class="close-reveal-modal">&#215;</a>
        </div>
                    </li>
                    <?php $query_3="SELECT sub_task_id,sub_task_name,cf_change FROM amz_sub_tasks where task_id='$sec_task_id' AND deletion=0";
                    ...
                    <li class="option-content"><?php echo $sec_stask_name=$result_row_3->sub_task_name?>&nbsp;<?php echo $task_cat?>
                            &nbsp;&nbsp;&nbsp;
                            <a href="#" data-reveal-id="edit_sub_task_<?php echo $sec_stask_id ?>" data-reveal-ajax="./edit_data.php?subject=subtask&&val=<?php echo $sec_stask_id ?>">Edit</a>&nbsp;&nbsp;&nbsp;
                            <a href="#" onclick="del_ua('subtask','<?php echo $sec_stask_id?>');">Delete</a>
                            </li>
                                <div id="edit_sub_task_<?php echo $sec_stask_id ?>" class="reveal-modal small" data-reveal>
                                    <a class="close-reveal-modal">&#215;</a>
                                </div>
                            </li>
                            <?php $query_5="SELECT tdi_no,task_info FROM amz_task_desc where task_id='$sec_task_id'  AND deletion=0";
                            ...
                            <li class="td_style"><?php echo $result_row_5->task_info?>
                              &nbsp;&nbsp;&nbsp;
                            <a href="#" data-reveal-id="edit_task_des_<?php echo $task_des_id ?>" data-reveal-ajax="./edit_data.php?subject=td&&val=<?php echo $task_des_id ?>">Edit</a>&nbsp;&nbsp;&nbsp;
                            <a href="#" onclick="del_ua('taskdes','<?php echo $task_des_id?>');">Delete</a>
                            </li>
                                <div id="edit_task_des_<?php echo $task_des_id ?>" class="reveal-modal small" data-reveal>
                                    <a class="close-reveal-modal">&#215;</a>
                                </div>
                                    </li>
                               <?php }
...
<style>
    #hide_show_st{background:#ddd;padding-left:25px;margin-top:2px;cursor:pointer;width:70%;}
  .option-content{background:#eee;padding-left:25px;margin-top:2px;cursor:pointer;width:50%;}
  .team_style {padding-left:25px;margin-top:2px;cursor:pointer;background-color: #d7cfcf; width: 100%}
  .td_style {padding-left:25px;margin-top:2px;cursor:pointer;background-color: #ffeeee;width:30%;}
</style>
