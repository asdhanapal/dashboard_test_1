...
$( document ).ready(function() {
    ...
    view_report_1_get_teams();
});

function view_report_1_get_teams()
{
    var year=$('#year').val();
    $('#user_tasks_list_1').html('<center><img src="../img/loading.gif"> Loading Team wise report...</center>');
    $("#user_tasks_list_1").load( "<?php echo _BACK_TO_PRE_.COMMON.YEARLY_VIEW_PROD_DATA_D1?>",  { "year": year},function( response, status, xhr ) {
    if ( status === "success" )
    {
...
function view_report_1_get_full_info(team_id)
{
    var year=$('#year').val();
    $('#user_tasks_list_1').html('<center><img src="../img/loading.gif"> Loading Team wise report...</center>');
    $("#user_tasks_list_1").load( "<?php echo _BACK_TO_PRE_.COMMON.YEARLY_VIEW_PROD_DATA_D1?>",  { "year": year},function( response, status, xhr ) {
...
function view_report_2_get_teams()
{
    ...
    $("#user_tasks_list_2").load( "<?php echo _BACK_TO_PRE_.COMMON.YEARLY_VIEW_PROD_DATA_D2_1?>",  { "year": year},function( response, status, xhr ) {
    if ( status === "success" )
    {
        $('#loading_user_data').html('');
    }
...
function view_report_2_get_full_info(team_id)
{
    ...
    $.ajax({
        type: "POST",
        url: "<?php echo _BACK_TO_PRE_.COMMON.YEARLY_VIEW_PROD_DATA_D2_2?>",
        data:data,
...
function view_report_3()
{
    var year=$('#year').val();
    $('#user_tasks_list_1').html('<center><img src="../img/loading.gif"> Loading Team wise report...</center>');
    $("#user_tasks_list_1").load( "<?php echo _BACK_TO_PRE_.COMMON.YEARLY_VIEW_PROD_DATA_D3?>",  { "year": year},function( response, status, xhr ) {
    if ( status === "success" )
    {
        $('#loading_user_data').html('');
    }
...
//function call_hide_all()
//{
//  $('tr[class^="team_"]').hide('slow');
//}

function show_hide(id)
{
    $('#display_team_tr_'+id).toggle('slow');
//    if($('.team_'+id).css('display') === 'none') {
//        $('.team_'+id).show('slow');
//    } else {
//        $('.team_'+id).hide('slow');
//    }
...
//        background: #fff;
    ...
    .project_odd:hover, .project_even:hover  {
        //background-image: url(../css/svg_gradient.php?from=ced6df&to=b6c6d7);
