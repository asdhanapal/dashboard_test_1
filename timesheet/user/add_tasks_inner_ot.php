...
if($have_dsi==1 && $dsi_type==1)
{
    ...
    $count=$_POST['count'];
    if($device_count)
            $query_user_task="INSERT INTO user_tasks_ot(user_id,team_id,build,tasks_id,sub_task_id,task_desc,count,noofdevice,time,date,cmds,create_date,maintain_date) VALUES('$user_id','$team','$build','$task',$sub_task,'$task_desc','$count','$noofdevice','$time','$date','$cmds',now(),now())";
    else
            $query_user_task="INSERT INTO user_tasks_ot(user_id,team_id,build,tasks_id,sub_task_id,task_desc,count,time,date,cmds,create_date,maintain_date) VALUES('$user_id','$team','$build','$task',$sub_task,'$task_desc','$count','$time','$date','$cmds',now(),now())";
}
else if($have_dsi==0 || $dsi_type!=1)
{
    $count=$_POST['count'];
    if($device_count)
            $query_user_task="INSERT INTO user_tasks_ot(user_id,team_id,build,tasks_id,sub_task_id,count,noofdevice,time,date,cmds,create_date,maintain_date) VALUES('$user_id','$team','$build','$task',$sub_task,'$count','$noofdevice','$time','$date','$cmds',now(),now())";
    else 
            $query_user_task="INSERT INTO user_tasks_ot(user_id,team_id,build,tasks_id,sub_task_id,count,time,date,cmds,create_date,maintain_date) VALUES('$user_id','$team','$build','$task',$sub_task,'$count','$time','$date','$cmds',now(),now())";
}
