...
//    echo '<optgroup label="-- DA\'s --"></option>';
    echo '<option value=\"\" disabled=\"\">-- DA\'s --</option>';
    if(mysqli_num_rows($result_get_tasks))
    {
        ...
        echo "<option value=\"\" disabled=\"\"> --No DA\'s available --</option>";
    
    $query_get_tasks="select user_id,user_name FROM amz_login WHERE user_type='2' ORDER BY user_name ASC";
    $result_get_tasks= $conn->runsql($query_get_tasks,$dbcon);
    
    //echo '<optgroup label="-- SDA\'s --"></option>';
    echo '<option disabled>-- SDA\'s --</option>';
    if(mysqli_num_rows($result_get_tasks))
    {
        while($result_tasks=  mysqli_fetch_object($result_get_tasks))
        {
        ...
        echo "<option value=\"\" disabled=\"\"> --No SDA's available --</option>";
