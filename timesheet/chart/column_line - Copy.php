...
if(sizeof($team)!=1 || empty($team) || $team[0]=="null")
{
    ...
    $output['result_fail']="Please select exactly one team!";//die("Please select exactly one team for create chart!");
...
$var='{"chart": {"zoomType": "xy","type": "column","renderTo": "user_tasks_list"},"title": {        "text": "';
$var.=$title!=""?$title:$output['title'];
$var.='"},"subtitle": {        "text": "Sub title goes here"},    "xAxis": [{        "categories":[';
$var.=$task_list;
$var.='],        "crosshair": "true"    }],    "yAxis": [    {         "labels": {            "format": "{value}",            "style": {                "color": "Highcharts.getOptions().colors[1]"            }        },        "title": {            "text": "';
$var.=$x_axis==""?'Work Units (in %)':$x_axis;
$var.='",            "style": {                "color": "Highcharts.getOptions().colors[1]"            }        },        "min":"0",        "max":"';
$var.=$max;
$var.='"    }, {         "title": {            "text": "';
$var.=$x_axis==""?'Work Units (in %)':$x_axis;
$var.='",            "style": {                "color": "Highcharts.getOptions().colors[0]"            }        },        "labels": {            "format": "{value}",            "style": {                "color": "Highcharts.getOptions().colors[0]"            }        },       "opposite": "true",       "min":"0",        "max":"';
$var.=$max;
$var.='"    }],    "tooltip": {        "shared": "true"    },    "legend": {        "layout": "vertical",        "align": "left",        "x": "0",        "verticalAlign": "top",        "y": 0,        "floating": true           },    "series": [{        "name": "Actual",        "type": "column",        "yAxis": 1,        "data": [';
$var.=$wu_avg;
$var.='],        "tooltip": {            "valueSuffix": " %"        }    }, {        "name": "Target",        "type": "spline",       "data": [';
$var.=$daily_target_temp_in_wu_in_per;
$var.='],        "tooltip": {            "valueSuffix": " %"        }    }]}';
$output['result_s_data']="";
//$output['result_s_chart']=$var;
echo $var;
//echo json_encode($output);
?>
