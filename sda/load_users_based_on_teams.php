...
session_start();
//print_r($_SESSION);
$team_list=$_SESSION['team_id'];
//print_r($team_list);
echo $team= implode(",",$team_list);
...
if(mysqli_num_rows($result_get_tasks))
{
    ...
    {
        $user_id=$result_tasks->user_id;
        echo $query_check_user="SELECT user_id FROM amz_user_info WHERE user_id='$user_id' AND team_id IN($team)";
        $query_check_user_result= $conn->runsql($query_check_user,$dbcon);
        if(mysqli_num_rows($query_check_user_result))
        {
        ?>
