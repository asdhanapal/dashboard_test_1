...
function del_ua(id)
    {
        if(confirm('Are you sure to delete?'))
        {
            ...
            $.ajax({
                type: "POST",
                url: "del_user_entries_ot.php?action=del_ua",
                data: "id="+id,
...
<div class="row" style="width: 99%">
	...
	<input type="hidden" name="file_name" id="file_name" value="<?php echo $file_name?>">
        ...
        <input type="submit" name="Export_2_excel" id="Export_2_excel" onclick="" value="Export to Excel" class="button tiny" disabled="true" title="currently this function not available!">
</form>
</div>
    </div>
</div>
...
<table width="100%" class="tablesorter" id="report_view">
    ...
    if (mysqli_num_rows($result)) {
        while ($result_row = mysqli_fetch_object($result)) {
            ...
            <td align="center"><a href="#" data-reveal-id="myModal" data-reveal-ajax="./view_user_entries_ot.php?val=<?php echo $result_row->task_id?>">View</a>&nbsp;&nbsp;&nbsp;<a href="#" onclick="del_ua('<?php echo $result_row->task_id?>');">Delete</a></td>
        </tr>
