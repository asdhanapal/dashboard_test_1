            ...
            <div class="row">
                ...
                <div id="row">
                    <div class="large-3 large-offset-9 columns" style="text-align: right;">
                        <a onclick="column_line_with_target_raw_data();" id="link_for_data">Click here to view data for the below chart</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    $.ajax({
        ...
        success: function(msg)
        {
            ...
            if(msg['result_status']=="F")
            {
                ...
                $('#user_tasks_list').html("Currently support for single team only!:(");
            }
            else
            {
...
function column_line_with_target_raw_data()
{
    $('#loading_user_data').html('<center><img src="../img/loading.gif"></center>');
    var from_date=$("#date_from").val();
    var to_date=$("#date_to").val();
    ...
    $.ajax({
        type: "POST",
        url: "../chart/column_line_raw_data.php",
        //dataType: "json", 
        data: "from_date="+ from_date + "&to_date=" + to_date + "&report_view="+ report_view + "&team="+ team + "&task=" + task + "&user=" + user + "&title=" + title + "&x_axis=" + x_axis + "&y_axis=" + y_axis,
        success: function(msg)
        {
            $("#loading_user_data").hide().html(msg).fadeIn('slow');
            $('#link_for_data').html('Hide data');
            $("#link_for_data").attr("onclick","link_for_data()");
        }
    });
}

function link_for_data()
{
    $("#loading_user_data").hide().html('&nbsp;').fadeIn('slow');
    $('#link_for_data').html('Click here to view data for the below chart');
    $("#link_for_data").attr("onclick","column_line_with_target_raw_data()");
}
</script>
